...
create_app() {
  ...
  <array>
    <dict>
      ...
      <string>LDCodex Links</string>
      <key>CFBundleURLSchemes</key>
      <array>
        <string>ldcodex</string>
...
prepare_icon
prepare_background
create_app "LDCodex" "CodexPlusPlus" "$BINARY_DIR/codex-plus-plus" "cn.dicad.ldcodex" "true"
create_app "LDCodex 管理工具" "CodexPlusPlusManager" "$BINARY_DIR/codex-plus-plus-manager" "cn.dicad.ldcodex.manager" "false"

sign_app "$STAGE/LDCodex.app"
sign_app "$STAGE/LDCodex 管理工具.app"
...
hdiutil create -volname "LDCodex" -srcfolder "$STAGE" -ov -format UDRW "$DMG_WORK_PATH"
...
with timeout of 30 seconds
  tell application "Finder"
    ...
    tell dmgDisk
      ...
      set position of item "LDCodex.app" to {220, 390}
      set position of item "LDCodex 管理工具.app" to {460, 390}
    end tell
