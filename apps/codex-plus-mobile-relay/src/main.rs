...
async fn main() -> anyhow::Result<()> {
    let bind = env::var("CODEX_PLUS_MOBILE_RELAY_BIND")
        ...
        .unwrap_or_else(|| "0.0.0.0:58323".to_string());
    let listener = TcpListener::bind(&bind)
        .await
        .with_context(|| format!("failed to bind mobile relay server on {bind}"))?;
    let local_addr = listener.local_addr()?;
    println!("LDCodex mobile relay listening on ws://{local_addr}");
    println!(
...
<head>
  ...
  <title>LDCodex Mobile Relay</title>
  <style>
...
<body>
<div class="app">
  <header class="topbar"><strong class="title">LDCodex</strong><span id="status" class="status">待连接</span></header>
  <main class="layout">
...
async function ensureAppServer() {
  ...
  await rpcRaw("initialize", { clientInfo: { name: "LDCodex Mobile Relay", version: "1.0.0" }, capabilities: { experimentalApi: true } });
}
