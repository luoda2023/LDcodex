...
// ── Freebuff 配置（LDCodex 集成的 Freebuff 汉化/插件/模型管理） ──

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct FreeBuffStatusPayload {
...
#[derive(Debug, Clone, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct FreeBuffModelRequest {
    pub id: String,
...
impl From<FreeBuffModelRequest> for codex_plus_core::freebuff::FreeBuffModel {
    fn from(request: FreeBuffModelRequest) -> Self {
        Self {
            id: request.id,
            display_name: request.display_name,
            tagline: request.tagline,
            availability: request.availability,
            data_use: request.data_use,
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct FreeBuffModelsPayload {
    pub models: Vec<codex_plus_core::freebuff::FreeBuffModel>,
    pub path: String,
}

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct FreeBuffPatchPayload {
    pub exit_code: i32,
    pub output: String,
}

fn freebuff_install_dir() -> Option<PathBuf> {
    codex_plus_core::freebuff::detect_install_dir()
}

#[tauri::command]
pub fn freebuff_status() -> CommandResult<FreeBuffStatusPayload> {
    let Some(install_dir) = codex_plus_core::freebuff::detect_install_dir() else {
        return failed(
            "未检测到 Freebuff 安装目录。",
            FreeBuffStatusPayload {
                install_dir: codex_plus_core::freebuff::default_install_dir()
                    .to_string_lossy()
                    .into_owned(),
                installed: false,
                applied: 0,
    ...
    let status = codex_plus_core::freebuff::patch_status(&install_dir);
    let runtime = codex_plus_core::freebuff::find_runtime(&install_dir)
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_default();
    let payload = FreeBuffStatusPayload {
        ...
        total: status.get("total").and_then(Value::as_u64).unwrap_or(0) as usize,
        complete: status
            .get("complete")
    ...
    ok("Freebuff 状态已读取。", payload)
}

#[tauri::command]
pub fn freebuff_list_models() -> CommandResult<FreeBuffModelsPayload> {
    let Some(install_dir) = freebuff_install_dir() else {
        return failed(
            "未检测到 Freebuff 安装目录。请先安装 Freebuff 桌面版。",
            FreeBuffModelsPayload {
                models: Vec::new(),
                path: String::new(),
            },
        );
    };
    let payload = codex_plus_core::freebuff::list_models(&install_dir);
    let models = serde_json::from_value(
        payload
            .get("models")
            .cloned()
    ...
    let path = payload
        ...
        .unwrap_or_default()
        .to_string();
    ok("自定义模型已读取。", FreeBuffModelsPayload { models, path })
}

#[tauri::command]
pub fn freebuff_upsert_model(request: FreeBuffModelRequest) -> CommandResult<FreeBuffModelsPayload> {
    let Some(install_dir) = freebuff_install_dir() else {
        return failed(
            "未检测到 Freebuff 安装目录。请先安装 Freebuff 桌面版。",
            FreeBuffModelsPayload {
    ...
    let model = codex_plus_core::freebuff::FreeBuffModel::from(request);
    let payload = codex_plus_core::freebuff::upsert_model(&install_dir, &model);
    ...
    .unwrap_or_default();
    let path = codex_plus_core::freebuff::custom_models_path(&install_dir)
        .to_string_lossy()
        .into_owned();
    if status == "ok" {
        ok(message, FreeBuffModelsPayload { models, path })
    } else {
        failed(message, FreeBuffModelsPayload { models, path })
    }
}

#[tauri::command]
pub fn freebuff_delete_model(id: String) -> CommandResult<FreeBuffModelsPayload> {
    let Some(install_dir) = freebuff_install_dir() else {
        return failed(
            "未检测到 Freebuff 安装目录。请先安装 Freebuff 桌面版。",
            FreeBuffModelsPayload {
    ...
    let payload = codex_plus_core::freebuff::delete_model(&install_dir, &id);
    let status = payload.get("status").and_then(Value::as_str).unwrap_or("failed");
    let message = payload
        .get("message")
        .and_then(Value::as_str)
...
pub async fn freebuff_run_patch(action: String) -> CommandResult<FreeBuffPatchPayload> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        codex_plus_core::freebuff::run_patch(&install_dir, &action)
    })
    .await
    .unwrap_or_else(|error| codex_plus_core::freebuff::PatchRunResult {
        status: "failed".to_string(),
        message: format!("补丁引擎执行失败：{error}"),
        exit_code: -1,
        output: String::new(),
    });
    let payload = FreeBuffPatchPayload {
        exit_code: result.exit_code,
        output: result.output.clone(),
    };
    if result.status == "ok" {
        ok(&result.message, payload)
    } else if result.status == "partial" || result.status == "missing" {
        // check 类结果：以 warning 语义返回，但消息清晰
        ok(&result.message, payload)
    } else {
        failed(&result.message, payload)
    }
}
...
pub fn restart_codex_plus(request: LaunchRequest) -> CommandResult<Value> {
    ...
    match restart_codex_plus_after_stop(&request, &home, settings.as_ref(), spawn_silent_launcher) {
        ...
        Err(error) => {
            let message = format!("重启 LDCodex 失败：{error}");
            let _ =
...
fn managed_dream_skin_image_backup(
    ...
    if !codex_plus_core::dream_skin::is_managed_dream_skin_image(path, state_dir) {
        anyhow::bail!("Dream Skin image is not managed by LDCodex");
    }
...
pub fn import_local_session(path: String) -> CommandResult<SessionImportPayload> {
    ...
    match codex_plus_core::session_share::import_rollout_file(&home, &source_path) {
        Ok(result) => ok(
            "会话已导入 LDCodex。请刷新会话列表；如果仍未显示，请重启 Codex。",
            SessionImportPayload {
                session_id: result
                    .get("session_id")
                    .and_then(Value::as_str)
...
/// 自己的 LDCodex.exe，因为文件选择器只按 exe 扩展名过滤）。
///
...
mod tests {
    ...
    fn requested_launch_status_identifies_the_current_request() {
        ...
        assert_eq!(status.helper_port, Some(58322));
    ...
    fn startup_options_honors_show_update_argument() {
        assert!(should_show_update(
            ["LDCodexManager.exe", "--show-update"],
            None
        ));
    }
...
"#;
    ...
    #[test]
    /// #1972：用户误把 LDCodex 自己的 exe 选成了「Codex 应用路径」——文件选择器
    ...
    fn normalize_settings_before_save_drops_an_invalid_codex_app_path() {
        let codex_plus_own_exe = if cfg!(windows) {
            r"D:\LDCodex\ldcodex.exe"
        } else {
            "/Applications/LDCodex/LDCodex"
        };
        let settings = BackendSettings {
