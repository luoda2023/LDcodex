...
fn macos_packager_hides_silent_launcher_but_not_manager() {
    ...
    assert!(script.contains(
        "create_app \"Codex++\" \"CodexPlusPlus\" \"$BINARY_DIR/LDCodex\" \"com.bigpizzav3.codexplusplus\" \"true\""
    ));
    assert!(script.contains(
        "create_app \"Codex++ 管理工具\" \"CodexPlusPlusManager\" \"$BINARY_DIR/LDCodexManager\" \"com.bigpizzav3.codexplusplus.manager\" \"false\""
    ));
}
