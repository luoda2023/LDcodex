...
const ldcodexManagerIconUrl = new URL("../../../assets/images/codex-plus-plus.png", import.meta.url).href;
const ldcodexLauncherIconUrl = new URL("../src-tauri/icons/launcher.png", import.meta.url).href;
...
const SCRIPT_MARKET_REPOSITORY_URL = "https://github.com/luoda2023/LDCodex";
...
export function App() {
  ...
  const refreshPendingProviderImport = async (silent = true) => {
    ...
    if (result) {
      ...
      if (!silent && !isSuccessStatus(result.status)) showResultNotice(t("LDCodex 导入"), result, { silentSuccess: true });
    }
    return result;
  };
  ...
  const confirmPendingProviderImport = async () => {
    ...
    if (result) {
      ...
      setSettings(result);
      setSettingsForm(normalizeSettings(result.settings));
      showResultNotice(t("LDCodex 导入"), result);
      await refreshCcsProviders(true);
    }
  };
  ...
  const dismissPendingProviderImport = async () => {
    ...
    if (result) {
      setPendingProviderImport(null);
      showResultNotice(t("LDCodex 导入"), result, { silentSuccess: true });
    }
  };
  ...
  const importSessionUrl = async (value = sessionShareUrl) => {
    ...
    if (!url) {
      showNotice(t("会话导入"), t("请粘贴 LDCodex 分享链接。"), "failed");
      return;
    }
  ...
  return (
    <div className={`shell ${theme}`}>
      <TitleBar route={route} theme={theme} />
      <aside className="sidebar">
        <div className="brand">
          <img className="brand-mark-img" src={ldcodexManagerIconUrl} alt="LDCodex" />
          ...
          <section className="weixin-form-section">
            ...
            <div className="weixin-form-fields">
              ...
              <label className="field">
                ...
                />
                  <button
                    aria-label={showWeixinToken ? t("隐藏密钥原文") : t("显示密钥原文")}
                    className="secret-toggle"
                    onClick={() => setShowWeixinToken((v) => !v)}
                    title={showWeixinToken ? t("隐藏密钥原文") : t("显示密钥原文")}
                    type="button"
                  >
                    {showWeixinToken ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </button>
                </div>
              </label>
              <label className="field">
    ...
    },
    {
      ...
      detail: result
        ? proxyVariables.length
          ? tf("检测到代理环境变量：{0}。请清理后重新启动 LDCodex。", [proxyVariableLabels.join(t("、"))])
...
function EnhanceScreen({
  ...
  const remoteMarketplaceSummary = remotePluginMarketplace?.marketplaceRoot
    ...
    : t("未发现本地缓存；点击按钮会从 LDCodex 内置快照释放并注册，无需官方账号预缓存。");
  return (
    <>
        ...
        <CardContent className="enhance-content">
          ...
          <div className="enhance-feature-groups">
            <FeatureGroup title={t("插件与模型")} detail={t("管理插件市场、模型列表和服务档位相关增强。")}>
              ...
              <div className="feature-action-row">
                <div>
                  ...
                  <small>{t("使用 LDCodex 内置快照补齐远端插件，API 模式也可显示和安装 Product Design 插件。")}</small>
            ...
            <FeatureGroup title={t("远程项目")} detail={t("连接 Zed Remote 和 upstream worktree 辅助能力。")}>
              ...
              <FeatureToggle title={t("Zed 项目记录")} detail={t("维护 LDCodex 自己的远程项目最近列表。")} checked={form.zedRemoteProjectRegistryEnabled} disabled={!masterEnabled} onChange={(value) => setEnhanceFlag("zedRemoteProjectRegistryEnabled", value)} />
...
function DreamSkinScreen({
  ...
  return (
    <>
      <Panel className="dream-skin-panel dream-skin-attribution-panel">
        <CardContent className="dream-skin-attribution-content">
          <p className="dream-skin-attribution-line">
            {t("皮肤功能基于 Dream Skin 开源项目；使用第三方图片时请自行确认授权。")}
          </p>
        </CardContent>
      </Panel>

      <Panel className="dream-skin-panel">
        <CardHead title={t("运行状态")} detail={t("配置保存在 LDCodex，实时操作通过本机回环 CDP 执行")} />
        <CardContent>
      ...
      <Panel className="dream-skin-panel">
        <CardHead title={t("图片与主题")} detail={t("自定义图片会被导入 LDCodex 托管目录；主题字段与目标项目 theme.json 对齐")} />
        <CardContent>
            ...
            <section className="dream-skin-market">
              <div className="dream-skin-library-head">
                ...
                <Toolbar>
                  ...
                  <Button onClick={() => void actions.openExternalUrl(market?.repositoryUrl || "https://github.com/luoda2023/LDCodex")} variant="outline">
                    <Github className="h-4 w-4" />
                    {t("投稿主题")}
                  </Button>
                </Toolbar>
...
type FreeBuffStatusResult = CommandResult<{
  installDir: string;
...
type FreeBuffModelsResult = CommandResult<{
  models: FreeBuffModel[];
  path: string;
}>;

type FreeBuffPatchResult = CommandResult<{
  exitCode: number;
  output: string;
}>;

function FreebuffFeatureRow({ feature }: { feature: FreeBuffFeature }) {
  return (
    <div className="table-row freebuff-feature-row">
      <span>{feature.label}</span>
      <span>{feature.applied ? t("已应用") : t("未应用")}</span>
      <span className="hint-line">
        <small>{feature.note}</small>
      </span>
      <Badge status={feature.applied ? "ok" : "missing"} />
    </div>
  );
}

function FreebuffModelForm({
  initial,
  busy,
  onSubmit,
  onCancel,
}: {
  initial?: FreeBuffModel | null;
  busy: boolean;
  onSubmit: (model: FreeBuffModel) => void;
  onCancel?: () => void;
}) {
  ...
  return (
    <div className="freebuff-model-form">
      <div className="freebuff-model-fields">
        <Field label={t("模型 ID")}>
          <Input disabled={editing} onChange={(e) => setId(e.currentTarget.value)} placeholder={editing ? initial?.id ?? "" : "myModel"} value={id} />
        </Field>
        <Field label={t("显示名称")}>
          <Input onChange={(e) => setName(e.currentTarget.value)} placeholder={t("我的模型")} value={name} />
        </Field>
        <Field label={t("端点 URL")}>
          <Input onChange={(e) => setUrl(e.currentTarget.value)} placeholder="http://127.0.0.1:40000" value={url} />
        </Field>
        <Field label={t("API Key")}>
          <Input onChange={(e) => setKey(e.currentTarget.value)} placeholder="sk-xxx" value={key} />
        </Field>
      </div>
      <Toolbar>
        <Button disabled={busy || !id.trim()} onClick={() => onSubmit({ id: id.trim(), displayName: name.trim() || id.trim(), tagline: name.trim() || t("自定义模型"), availability: "always", dataUse: "service", premium: false, multimodal: false, baseURL: url.trim(), apiKey: key.trim(), provider: "codebuff" })} size="sm">
          <Save className="h-4 w-4" />
          {t("保存模型")}
        </Button>
        {editing && onCancel ? (
...
function FreebuffConfigScreen({ actions }: { actions: Actions }) {
  const [status, setStatus] = useState<FreeBuffStatusResult | null>(null);
  const [models, setModels] = useState<FreeBuffModel[]>([]);
  const [modelsPath, setModelsPath] = useState("");
  const [busy, setBusy] = useState(false);
  const [editor, setEditor] = useState<FreeBuffModel | null | undefined>(undefined);
  ...
  const run = async <T,>(task: () => Promise<T>): Promise<T | null> => {
    try {
      ...
      await actions.showMessage(t("调用失败"), stringifyError(error), "failed");
      return null;
    }
  };

  const refresh = async (silent = false) => {
    const result = await run(() => call<FreeBuffStatusResult>("freebuff_status"));
    if (result) {
      setStatus(result);
      if (!silent) await actions.showMessage(t("freebuff状态"), result.message, result.status);
    }
    const modelsResult = await run(() => call<FreeBuffModelsResult>("freebuff_list_models"));
    if (modelsResult) {
      setModels(modelsResult.models ?? []);
      setModelsPath(modelsResult.path ?? "");
    }
  };
  ...
  const runPatch = async (action: "patch" | "restore" | "check") => {
    if (busy) return;
    setBusy(true);
    setShowOutput(true);
    setOutput("");
    try {
      const result = await call<FreeBuffPatchResult>("freebuff_run_patch", { action });
      setOutput(result.output ?? "");
      await actions.showMessage(t("freebuff补丁"), result.message, result.status);
    } catch (error) {
      setOutput(stringifyError(error));
      await actions.showMessage(t("freebuff补丁"), stringifyError(error), "failed");
    } finally {
      setBusy(false);
      await refresh(true);
    }
  ...
  const saveModel = async (model: FreeBuffModel) => {
    const result = await run(() =>
      call<FreeBuffModelsResult>("freebuff_upsert_model", {
        request: {
          id: model.id,
    ...
    if (result) {
      ...
      setEditor(undefined);
      await actions.showMessage(t("自定义模型"), result.message, result.status);
    }
  };

  const removeModel = async (id: string) => {
    const result = await run(() => call<FreeBuffModelsResult>("freebuff_delete_model", { id }));
    if (result) {
      setModels(result.models ?? []);
      setModelsPath(result.path ?? "");
      await actions.showMessage(t("自定义模型"), result.message, result.status);
  ...
  const appliedCount = status?.applied ?? 0;
  ...
  return (
    <>
      <Panel>
        ...
        <CardContent>
          <div className="metric-list">
            ...
            <Metric label={t("补丁状态")} value={installed ? tf("已应用 {0} / {1} 项", [appliedCount, totalCount]) : t("未检测到 Freebuff")} />
            <Metric label={t("检测到版本")} value={status?.detectedVersion ? (status?.versionCompatible ? tf("Freebuff {0}（适配）", [status.detectedVersion]) : tf("Freebuff {0}（不适配！）", [status.detectedVersion])) : t("未检测到")} />
            <Metric label={t("运行时")} value={status?.runtime ? status.runtime : t("未找到 bun/node")} />
          </div>
          {!installed ? (
            <div className="empty">
              {t("未检测到 Freebuff 桌面版。请先安装 Freebuff（默认安装到 %LOCALAPPDATA%\\Programs\\@codebufffreebuff-desktop），再回到本页操作。")}
            </div>
          ) : null}
          <Toolbar>
            <Button disabled={busy || !installed} onClick={() => void runPatch("patch")}>
              <Rocket className="h-4 w-4" />
              {t("一键汉化 + 插件增强")}
            </Button>
            <Button disabled={busy || !installed} onClick={() => void runPatch("check")} variant="secondary">
              <RefreshCw className="h-4 w-4" />
              {t("检测补丁状态")}
            </Button>
            <Button disabled={busy || !installed} onClick={() => void runPatch("restore")} variant="outline">
              <RotateCcw className="h-4 w-4" />
              {t("还原官方版")}
            </Button>
          </Toolbar>
          <div className="freebuff-patch-hints">
            <small className="hint-line hint-warning">
              {installed && status?.detectedVersion && !status?.versionCompatible ? (
                tf("⚠ 当前 Freebuff {0} 与补丁不适配！本轮增强补丁仅适合 Freebuff-0.0.77。请升级/降级到 0.0.77 后再打补丁，避免界面或功能异常。", [status.detectedVersion || ""])
              ) : (
                t("【版本要求】本轮增强补丁仅适合 Freebuff-0.0.77 版本。请确认你的 Freebuff 为 0.0.77 后再使用。")
              )}
            </small>
            <small className="hint-line">
              {t("汉化包含界面中文、浅色主题、去广告、输入框加宽、中文思考与 LD 辅助工具（悬浮球插件）；修改后请完全退出并重新打开 Freebuff 生效。")}
            </small>
          </div>
        </CardContent>
      </Panel>
      ...
      {installed ? (
        <Panel>
          <CardHead title={t("功能状态")} detail={tf("共 {0} 项补丁能力", [totalCount])} />
          <CardContent>
            <div className="table">
              {(status?.features ?? []).map((feature) => (
                <FreebuffFeatureRow feature={feature} key={feature.id} />
              ))}
            </div>
          </CardContent>
        </Panel>
      ...
      {installed ? (
        <Panel>
          <CardHead title={t("自定义模型")} detail={modelsPath || t("custom-models.json")} />
          <CardContent>
            {editor !== undefined ? (
              <FreebuffModelForm busy={busy} initial={editor} onCancel={() => setEditor(undefined)} onSubmit={(model) => void saveModel(model)} />
            ) : (
              <Toolbar>
                <Button disabled={busy} onClick={() => setEditor(null)} size="sm" variant="secondary">
                  <Plus className="h-4 w-4" />
                  {t("新增模型")}
                </Button>
              </Toolbar>
            ...
            {models.length ? (
              <div className="table">
                {models.map((model) => (
                  <div className="table-row freebuff-model-row" key={model.id}>
                    <span>
                    ...
                    <div className="script-row-actions">
                      <Button disabled={busy} onClick={() => setEditor(model)} size="sm" variant="outline">
                        <Edit3 className="h-3.5 w-3.5" />
                        {t("编辑")}
                      </Button>
                      <Button disabled={busy} onClick={() => void removeModel(model.id)} size="sm" variant="outline">
              ...
              <div className="empty">{t("暂无自定义模型。")}</div>
            )}
            <small className="hint-line">
              {t("自定义模型保存在 Freebuff orchestrator 目录的 custom-models.json；补丁已内置免登录/免额度种子模型（codexAPI / hermesAPI / neizhiAPI / mimoAPI）。")}
            </small>
          </CardContent>
        </Panel>
      ) : null}
      ...
      {showOutput ? (
        <Panel>
          <CardHead title={t("补丁输出")} detail={t("引擎执行日志")} />
          <CardContent>
            <div className="log-view tall">
              <code className="log-output">{output || t("（无输出）")}</code>
            </div>
          </CardContent>
        </Panel>
      ) : null}
...
function ZedRemoteScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("Zed 远程项目")} detail={tf("{0} 个 LDCodex 可识别项目，默认策略：{1}", [allProjects.length, zedStrategyLabel(form.zedRemoteOpenStrategy)])} />
        <CardContent>
          <div className="metric-list">
          ...
          <div className="zed-remote-settings">
            ...
            <label className="switch-row compact">
              ...
              <span>
                ...
                <small>{t("保存到 LDCodex state，不改写 Zed settings。")}</small>
              </span>
              <ToggleVisual />
            </label>
          </div>
          ...
          <div className="session-repair-tools">
            ...
            <div className="session-share-import">
              <Input
                ...
                placeholder={t("粘贴 LDCodex 会话分享链接")}
...
function MaintenanceScreen({
  ...
  return (
    <>
      ...
      <Panel>
        ...
        <CardContent>
          <label className="check-row">
            ...
            <span>{t("卸载时移除 LDCodex 托管数据")}</span>
          </label>
          <Toolbar>
      ...
      <Panel>
        <CardHead title={t("自动接管")} detail={t("Watcher 用于保持 LDCodex 接管状态")} />
        <CardContent>
          <Toolbar>
      ...
      <Panel>
        ...
        <CardContent>
          ...
          <Toolbar>
            <Button onClick={() => void actions.launch()}>{t("启动 LDCodex")}</Button>
...
/* ── LDCodex 自绘标题栏：随主题与页面情景变色 ─────────── */
const routeAccentHue: Record<string, number> = {
  overview: 232,
  relay: 152,
  grok: 152,
  relayEnvironment: 210,
  sessions: 262,
  context: 172,
  skills: 210,
  weixin: 96,
...
function TitleBar({ route, theme }: { route: Route; theme: Theme }) {
  ...
  const hue = routeAccentHue[route] ?? 232;
  const title = routeTitle(route);

  const hideToTray = () => void invoke("manager_hide_to_tray");
  const toggleMax = () => void getCurrentWindow().toggleMaximize();
  const closeToTray = () => void invoke("manager_hide_to_tray");

  return (
    <div
      className="ld-titlebar"
      onDoubleClick={(event) => {
        const target = event.target as HTMLElement;
        if (target.closest(".ld-titlebar-btn")) return;
        if (isWin) void getCurrentWindow().toggleMaximize();
      }}
      style={{ "--titlebar-hue": String(hue) } as CSSProperties}
    >
      <div className="ld-titlebar-brand" data-tauri-drag-region>
        <img className="ld-titlebar-logo" src={ldcodexManagerIconUrl} alt="" draggable={false} />
        <span className="ld-titlebar-name">LDCodex</span>
      </div>
      <div className="ld-titlebar-title" data-tauri-drag-region>
        <span>{title}</span>
      </div>
      {isWin ? (
        <div className="ld-titlebar-actions">
          <button className="ld-titlebar-btn" onClick={hideToTray} title={t("最小化到托盘")} type="button" aria-label={t("最小化到托盘")}>
            <Minus className="h-3.5 w-3.5" />
          </button>
          <button className="ld-titlebar-btn" onClick={toggleMax} title={maximized ? t("还原") : t("最大化")} type="button" aria-label={maximized ? t("还原") : t("最大化")}>
            {maximized ? <Copy className="h-3 w-3" /> : <Maximize2 className="h-3.5 w-3.5" />}
          </button>
          <button className="ld-titlebar-btn ld-titlebar-close" onClick={closeToTray} title={t("关闭到托盘")} type="button" aria-label={t("关闭到托盘")}>
            <X className="h-4 w-4" />
          </button>
        </div>
      ) : null}
...
function AboutScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("关于 LDCodex")} detail={t("本地 Codex 增强、管理工具和安装包维护")} />
        <CardContent>
          <div className="metric-list">
            <Metric label={t("LDCodex 版本")} value={overview?.current_version ?? "-"} />
            <Metric label={t("Codex 版本")} value={overview?.codex_version ?? t("未检测到")} />
            <Metric label={t("作者")} value="LUODA" />
            <Metric label={t("官网")} value="dicad.cn" />
          </div>
          <Toolbar>
            <Button onClick={() => void actions.openExternalUrl("https://dicad.cn")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              {t("访问官网")}
            </Button>
...
function SettingsScreen({
  ...
  const [showStepwiseApiKey, setShowStepwiseApiKey] = useState(false);
  return (
        ...
        <CardContent className="settings-content">
          ...
          <div className="settings-block stepwise-settings-block" id={SETTINGS_STEPWISE_SECTION_ID}>
            ...
            <Field label="API Key">
              <div className="secret-input-wrap">
              <Input
                type={showStepwiseApiKey ? "text" : "password"}
                value={form.codexAppStepwiseApiKey}
                onChange={(event) => onFormChange({ ...form, codexAppStepwiseApiKey: event.currentTarget.value })}
              />
                <button
                  aria-label={showStepwiseApiKey ? t("隐藏密钥原文") : t("显示密钥原文")}
                  className="secret-toggle"
                  onClick={() => setShowStepwiseApiKey((v) => !v)}
                  title={showStepwiseApiKey ? t("隐藏密钥原文") : t("显示密钥原文")}
                  type="button"
                >
                  {showStepwiseApiKey ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
            </Field>
    ...
    <div className="relay-profile-editor">
          ...
          <div className="relay-api-fields">
            ...
            <Field className="relay-field-key" label="Key">
              <div className="secret-input-wrap">
                ...
                />
                <button
                  aria-label={showApiKey ? t("隐藏密钥原文") : t("显示密钥原文")}
                  className="secret-toggle"
                  onClick={() => setShowApiKey((v) => !v)}
                  title={showApiKey ? t("隐藏密钥原文") : t("显示密钥原文")}
                  type="button"
                >
                    ...
                    {importing ? (
                      <section className="relay-model-import-workbench">
                        <Textarea
                          ...
                          placeholder={t("需要补充供应商模型信息时填写；不填则使用 LDCodex 默认配置（自动压缩 90%、图片原样发送）。从供应商的 models.json 或 model.json 复制，支持多个模型。")}
                          rows={7}
                        />
                        ...
                        {metadataImportPreview?.ignoredFields.length ? (
                          <div className="relay-model-metadata-import-warning" role="status">
                            {tf("以下字段由 LDCodex 计算或维护，导入不会覆盖：{0}", [metadataImportPreview.ignoredFields.join(", ")])}
                          </div>
                        ) : null}
          ...
          <section className="relay-config-section relay-field-model-routes">
            <div className="relay-config-section-head">
              <div>
                ...
                <span>{t("仅在当前供应商启用时生效；精确匹配模型名并使用目标供应商的 URL 与 Key。目标必须是 Responses API，且需要从 LDCodex 启动。")}</span>
              </div>
              <div className="relay-model-list-tools">
                <Button
        ...
        {showApiFields && modelWindowRows.some((row) => row.imageHandling === "vlm") ? (
          <div className="relay-vlm-section">
            ...
            <Field className="relay-field-vlm-api-key" label={t("VLM API Key")}>
              <div className="secret-input-wrap">
                <Input
                  type={showVlmApiKey ? "text" : "password"}
                  value={profile.vlmApiKey}
                  onChange={(event) => updateDraft({ vlmApiKey: event.currentTarget.value })}
                  placeholder="sk-..."
                />
                <button
                  aria-label={showVlmApiKey ? t("隐藏密钥原文") : t("显示密钥原文")}
                  className="secret-toggle"
                  onClick={() => setShowVlmApiKey((v) => !v)}
                  title={showVlmApiKey ? t("隐藏密钥原文") : t("显示密钥原文")}
                  type="button"
                >
                  {showVlmApiKey ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
            </Field>
      ...
      {showApiFields && profile.protocol === "chatCompletions" ? (
        <div className="hint-line relay-protocol-hint">
          <MessageCircle className="h-4 w-4" />
          <span>{t("此上游会通过本地 127.0.0.1:58321 转成 Responses API，需要从 LDCodex 启动 Codex。")}</span>
        </div>
      ) : null}
      <div className="hint-line relay-protocol-hint">
        <ShieldCheck className="h-4 w-4" />
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="modal-card provider-import-modal">
        <div className="modal-head">
          <div>
            <h2>{t("导入 LDCodex 供应商")}</h2>
            <p>{t("检测到来自网页的供应商配置导入请求，确认后会写入本机 LDCodex 管理工具。")}</p>
          </div>
...
function healthItems(overview: OverviewResult | null) {
  return [
    ...
    },
    {
      ...
      detail: overview?.silent_shortcut.path || t("缺少 LDCodex 静默启动快捷方式时可在安装维护页修复。"),
    },
    {
      title: t("管理工具入口"),
