...
/// issue #1933：管理器「重启」先强杀旧 launcher 再拉新的，旧 helper 交还 58321 要一小会儿。
...
async fn fixed_protocol_proxy_port_waits_for_the_previous_helper_to_release_it() {
    ...
    assert_eq!(
        events
            .iter()
            .filter(|event| event.as_str() == "start-helper-busy:58321")
            .count(),
        3
    );
    assert!(events.contains(&"start-helper:58321".to_string()));
...
async fn a_permanently_busy_protocol_proxy_port_reports_what_the_user_should_do() {
    ...
    assert!(message.contains("58321"), "unexpected message: {message}");
    assert!(
...
async fn launch_starts_helper_when_model_routing_is_enabled() {
    ...
    let start = before_stop
        .iter()
        .position(|event| event == "start-helper:58321")
        .unwrap();
    assert!(ensure < start);
    assert!(!before_stop.contains(&"inject:9229:58321".to_string()));
