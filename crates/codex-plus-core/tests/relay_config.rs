...
openai_base_url = "http://127.0.0.1:58321/v1"
...
"#
    ...
    assert!(live.contains(r#"openai_base_url = "http://127.0.0.1:58321/v1""#));
...
"#,
    ...
    assert!(ensure_active_protocol_proxy_config_in_home(temp.path(), &settings).unwrap());
    let updated = std::fs::read_to_string(config_path).unwrap();
    assert!(updated.contains(r#"openai_base_url = "http://127.0.0.1:58321/v1""#));
    assert!(updated.contains("[model_providers.custom]"));
    assert!(updated.contains(r#"base_url = "http://127.0.0.1:58321/v1""#));
    assert!(updated.contains(r#"experimental_bearer_token = "sk-preserve""#));
}
...
fn pure_api_profile_does_not_write_remote_control_openai_base_url() {
    let mut profile = RelayProfile {
        ...
        config_contents: r#"openai_base_url = "http://127.0.0.1:58321/v1"
model_provider = "custom"
"#
        .to_string(),
        ..RelayProfile::default()
...
"#
    ...
    assert!(live_config.contains(r#"base_url = "http://127.0.0.1:58321/v1""#));
...
"#
    ...
    assert!(!profile.config_contents.contains("127.0.0.1:58321"));
}
...
fn apply_relay_config_file_accepts_utf8_bom_config() {
    ...
    assert!(result.configured);
    assert!(config.contains(r#"model_provider = "custom""#));
    assert!(config.contains("http://127.0.0.1:58321/v1"));
}
...
fn clear_relay_config_removes_only_managed_openai_base_url() {
    ...
    std::fs::write(
        ...
        r#"openai_base_url = "http://127.0.0.1:58321/v1"
model_provider = "custom"
