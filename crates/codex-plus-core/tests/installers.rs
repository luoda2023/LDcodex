...
fn windows_entrypoint_plan_contains_silent_and_manager_entrypoints() {
    ...
    assert_eq!(plan.manager_path, "C:/Tools/LDCodexManager.exe");
    assert_eq!(plan.silent_icon_path, "C:/Tools/ldcodex.exe");
    assert_eq!(
        plan.manager_icon_path,
        "C:/Tools/LDCodexManager.exe"
    );
...
fn macos_bundle_metadata_contains_silent_and_manager_apps() {
    let options = InstallOptions {
        ...
        manager_path: Some("/opt/LDCodex/LDCodexManager".into()),
        remove_owned_data: false,
    };
...
fn companion_binary_path_resolves_macos_silent_app_next_to_manager_app() {
    ...
    assert_eq!(
        companion,
        std::path::PathBuf::from("/Applications/LDCodex.app/Contents/MacOS/CodexPlusPlus")
    );
    assert_ne!(
        companion,
...
fn companion_binary_path_resolves_macos_manager_app_next_to_silent_app() {
    let silent_exe = std::path::Path::new("/Applications/LDCodex.app/Contents/MacOS/CodexPlusPlus");
...
fn macos_companion_launch_uses_bundle_ids_from_app_translocation() {
    let manager_exe = std::path::Path::new(
        "/private/var/folders/x/AppTranslocation/manager-id/d/LDCodex 管理工具.app/Contents/MacOS/CodexPlusPlusManager",
    );
    let silent_exe = std::path::Path::new(
        "/private/var/folders/x/AppTranslocation/silent-id/d/LDCodex.app/Contents/MacOS/CodexPlusPlus",
    );
...
fn macos_companion_launch_keeps_bare_binary_development_mode() {
    let manager_exe = std::path::Path::new("/tmp/target/debug/LDCodexManager");
...
fn macos_companion_path_falls_back_to_workspace_release_launcher() {
    ...
    let bundle_exe = root.path().join(
        "target/release/bundle/macos/LDCodex.app/Contents/MacOS/LDCodexManager",
    );
    let release_launcher = root.path().join("target/release/LDCodex");
...
fn macos_bundle_does_not_wrap_the_bundle_executable_in_itself() {
    let options = InstallOptions {
        install_root: Some("/Applications".into()),
        launcher_path: Some("/Applications/LDCodex.app/Contents/MacOS/CodexPlusPlus".into()),
        manager_path: Some(
            "/Applications/LDCodex 管理工具.app/Contents/MacOS/CodexPlusPlusManager".into(),
        ),
        remove_owned_data: false,
    };
    ...
    assert_eq!(
        ...
        Some(std::path::PathBuf::from(
            "/Applications/LDCodex.app/Contents/MacOS/CodexPlusPlus"
        ))
    );
    assert_eq!(
