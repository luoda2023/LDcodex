...
<p align="center">
  <img src="docs/images/codex-plus-plus.png" alt="LDCodex icon" width="160">
</p>
...
<p align="center">
  <img alt="Release" src="https://img.shields.io/github/v/release/luoda2023/LDCodex">
  <img alt="Stars" src="https://img.shields.io/github/stars/luoda2023/LDCodex">
  <img alt="License" src="https://img.shields.io/github/license/luoda2023/LDCodex">
  <img alt="Rust" src="https://img.shields.io/badge/rust-1.85%2B-orange">
  <img alt="Tauri" src="https://img.shields.io/badge/tauri-2.x-24C8DB">
</p>

LDCodex is an external launcher and manager for the OpenAI Codex / ChatGPT desktop app. It uses the Chromium DevTools Protocol and a local helper for provider switching, protocol conversion, session management, and UI enhancements without modifying the official app's `app.asar` or installation files.

## Quick Start

Download the latest installer from [GitHub Releases](https://github.com/luoda2023/LDCodex/releases):

- Windows: `LDCodex-*-windows-x64-setup.exe`
- macOS Intel: `LDCodex-*-macos-x64.dmg`
- macOS Apple Silicon: `LDCodex-*-macos-arm64.dmg`

After installation, two entry points are available:

- `LDCodex`: silently starts the official desktop app with saved provider settings and enhancements.
- `LDCodex Manager`: manages providers, models, tools, sessions, enhancements, scripts, updates, and diagnostics.

For first-time setup, open the manager, verify the detected app path, configure a provider and optional enhancements, then launch through `LDCodex`. The Windows installer creates Desktop and Start Menu shortcuts. The macOS DMG installs `/Applications/LDCodex.app` and `/Applications/LDCodex 管理工具.app`.

## Community and Support

LDCodex is developed and maintained by LUODA. Website: <https://dicad.cn>; Repository: <https://github.com/luoda2023/LDCodex>. to report issues, share feedback, or suggest features.

WeChat: <a href="https://docs.qq.com/doc/DQ2VOanZTTFZJcUpZ#">get the latest group QR code</a>.

<img src="docs/images/discussion-group-qr.jpg" alt="LDCodex WeChat group QR code" width="260">

Telegram: <https://t.me/LDCodex>
...
Every UI enhancement is independently configurable. Disabling the global enhancement switch still leaves LDCodex available as a provider and launch manager.
...
Per-model windows accept values such as `1M`, `200K`, or plain integers. LDCodex generates a dedicated `model_catalog_json` for Codex.
...
Settings that depend on renderer injection generally require saving and restarting LDCodex.

## Updates and Packages

LDCodex publishes installers through GitHub Releases. Windows builds an NSIS installer, while macOS builds separate Intel x64 and Apple Silicon arm64 DMGs.
...
- LDCodex state and logs: `~/.codex-session-delete/`
- Provider Sync backups: `~/.codex/backups_state/provider-sync`

## FAQ

### The LDCodex menu does not appear

Launch through the `LDCodex` entry instead of opening the official app directly. Check the detected app path, launch status, and diagnostic logs in the manager's Maintenance and About pages.
...
LDCodex updates the remote branch first, then creates the worktree as if you ran:

```bash
git worktree add -b <new-branch> <worktree-path> upstream/<base-branch>
```

The new worktree starts from the fresh remote tracking branch instead of the local HEAD used by the current session. If LDCodex cannot safely recognize the current Codex version's native worktree form, use the LDCodex menu entry and enter the repository path, branch name, worktree path, remote, and base branch manually.
...
scripts/installer/
  windows/LDCodex.nsi     Windows NSIS installer
...
LDCodex is licensed under the [GNU Affero General Public License v3.0](LICENSE), SPDX identifier `AGPL-3.0-only`. Modified versions that are distributed or offered to users over a network must provide the corresponding source code as required by AGPLv3.

The license covers LDCodex code only. It does not grant rights to OpenAI, ChatGPT, Codex trademarks, application assets, or other third-party content.

## Compatibility

LDCodex depends on the official desktop app's page structure, CDP behavior, and local data formats. Official app updates may require injection updates. Keep backups before changing provider configuration or local session data.
